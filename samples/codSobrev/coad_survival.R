...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  coad  ####
##########################################################
##########################################################
setwd("E:/ArtigoJosivan/samples/coadNon")
...
clinical <- read.csv("coad_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
setwd("E:/ArtigoJosivan/survival/coadNon")


# Função para gerar gráficos e summary de testes de sobrevivencia

tumor="coad"
aux = dados[dados$Tumor == "coad",]
