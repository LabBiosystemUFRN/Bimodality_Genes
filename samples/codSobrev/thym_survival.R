...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  thym  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("thym_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
##########################################################
setwd("E:/ArtigoJosivan/samples/Non_log/thymNon")
...
menos <- c("EIF1AY_0.02_peak1_cluster1.lst","ZFY_0.02_peak1_cluster1.lst")
...
setwd("E:/ArtigoJosivan/survival/not_log/thym_Non")
##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("thym_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
clinical <- clinical[-c(1:4),]
colnames(clinical) <- clinical[1,]
