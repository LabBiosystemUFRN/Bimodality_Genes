...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  ucec  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("ucec_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
##########################################################
setwd("E:/ArtigoJosivan/samples/ucecNon")
...
setwd("E:/ArtigoJosivan/survival/ucecNon")


# Função para gerar gráficos e summary de testes de sobrevivencia

tumor="ucec"
aux = dados[dados$Tumor == "ucec",]
