...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  paad  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("paad_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
##########################################################
setwd("E:/ArtigoJosivan/samples/Non_log/paadNon")
...
setwd("E:/ArtigoJosivan/survival/not_log/paad_Non")
##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("paad_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
clinical <- clinical[-c(1:4),]
colnames(clinical) <- clinical[1,]
