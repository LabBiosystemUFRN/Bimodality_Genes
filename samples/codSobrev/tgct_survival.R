...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  tgct  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("tgct_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
##########################################################
setwd("E:/ArtigoJosivan/samples/Non_log/tgctNon")
...
menos <- c("POU5F1B_0.02_peak1_cluster1.lst","SYCP3_0.02_peak1_cluster1.lst")
...
setwd("E:/ArtigoJosivan/survival/not_log/tgct_Non")
##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("tgct_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
clinical <- clinical[-c(1:4),]
colnames(clinical) <- clinical[1,]
