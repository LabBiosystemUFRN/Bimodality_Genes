...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  lusc  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("lusc_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
#############################################################################
setwd("E:/ArtigoJosivan/samples/luscNon")
...
setwd("E:/ArtigoJosivan/survival/luscNon")


# Função para gerar gráficos e summary de testes de sobrevivencia

tumor="lusc"
aux = dados[dados$Tumor == "lusc",]
