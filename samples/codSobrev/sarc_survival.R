...
##########################################################
###   ORGANIZACÃO E FILTRAGEM DA BASE DE DADOS  sarc  ####
##########################################################

##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("sarc_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
...
##########################################################
setwd("E:/ArtigoJosivan/samples/Non_log/sarcNon")
...
menos <- c("RPS4Y1_0.02_peak1_cluster1.lst","ZDHHC15_0.02_peak1_cluster1.lst")
...
setwd("E:/ArtigoJosivan/survival/not_log/sarc_Non")
##### Dados clinicos TCGA - cbioportal
clinical <- read.csv("sarc_clinical_patient.txt", sep = '\t', header = F, stringsAsFactors = F)
clinical <- clinical[-c(1:4),]
colnames(clinical) <- clinical[1,]
